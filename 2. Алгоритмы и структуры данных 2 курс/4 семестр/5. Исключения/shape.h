...
public:
	...
	*/
	...
	void draw() { 
		if (w.x < 0 || w.y < 0 || e.x < 0 || e.y < 0 ||
			w.x > XMAX || w.y > YMAX || e.x > XMAX || e.y > YMAX) {
			throw Exception(w, e,
...
void rectangle::draw()
{
	if (sw.x < 0 || sw.y < 0 || ne.x < 0 ||ne.y < 0 ||
		...
		throw Exception(sw, ne, 
			"в функции-члене draw класса rectangle\nНехватка места на экране");
	}
	point nw(sw.x, ne.y);
	point se(ne.x, sw.y);
	put_line(nw, ne);
...
{
	if (lft.x < 0 || lft.y < 0 || rgt.x < 0 || rgt.y < 0 || up.x < 0 || up.y < 0
		|| up.y < lft.y || up.y < rgt.y) {
