					...
					switch (Q)
						{
							case 1:
									 	 ...
									 	 break;
							case 2:
										 mGuitars=fr(mGuitars);
										 NewGuitars=fr(NewGuitars);
										 NewCount = Count = 0; 		 
...
GUITARS* memory(GUITARS *mGuitars, int Count)
	{
...
GUITARS* enter(GUITARS* mGuitars, int *Count)
	{
	      ...
	      do
					{
		        (*Count)++;
		        mGuitars=memory(mGuitars, *Count);	
		        system("cls");
		...
		if(key==1)
		 {
				...
				for(i=0; i<Count; i++)
			     if(mGuitars[i].Year<=temp)
			       {
						 	(*NewCount)++;
							NewGuitars=memory(NewGuitars, *NewCount);
...
void output_struct(GUITARS *mGuitars, int Count)
	{
	    system("cls");
			...
			for(int i=0; i<Count; i++)
			    {
						printf("%12s  | %18d | %14d | %11s | %6s ",mGuitars[i].Name, mGuitars[i].Year, mGuitars[i].Strings, mGuitars[i].Wood.Deck, mGuitars[i].Wood.Neck);
						printf("\n================================================================================");
			    }
			system("pause");
	}
