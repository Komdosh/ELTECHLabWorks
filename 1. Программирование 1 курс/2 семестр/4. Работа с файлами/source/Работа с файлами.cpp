...
void freeStructure(LIST *list);                               //Прототип функции освобождения списка
...
int main() {
    ...
    do {
        ...
        switch (menuItem = enterNum(1, 4)) {
            case 1: // Добавление элементов в список
                enterMenu(&list);
                break;
            case 2: // Вывод списка
                outputMenu(list);
...
void outputList(LIST list, int Key) { //Key: 0 - вывод с начала, 1 - с конца
