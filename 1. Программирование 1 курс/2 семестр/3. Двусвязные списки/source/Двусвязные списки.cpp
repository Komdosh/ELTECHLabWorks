...
void freeStructure(LIST *list);                                          //Прототип функции очистка списка
    ...
    do {
        ...
        switch (menuItem = enterNum(1, 7)) {
            ...
            case 2: //Добавить элемент в список
                enterMenu(&list);
                break;
            case 3: //Удалить элемент из списка
                deleteMenu(&list);
                break;
            case 4: //Вывод списка
                if (chooseList())
                ...
                break;
            case 5: //Поиск
                freeStructure(&newList);
                newList = processingMenu(list);
                break;
            case 6: //Сортировка
                if (chooseList())
                    newList = sortMenu(newList);
                else
                    list = sortMenu(list);
                break;
            case 7: //Выход
...
LIST sortMenu(LIST list) {
    ...
    printf("Введите номер пункта - ");
    int menuItem = enterNum(1, 6);
    if (menuItem > 0 && menuItem < 6) {
        Sort = sort(list, menuItem);
        messages(4);
    }
    return Sort;
}
...
LIST sort(LIST list, int Key) {
    ...
    while (list) {
        temp = list;
        list = list->next;
        for (pFwd = sort, pBwd = NULL; pFwd && ((pFwd->Guitars.Name[0] > temp->Guitars.Name[0] && Key == 1) ||
                                                (temp->Guitars.Strings > pFwd->Guitars.Strings && Key == 2) ||
                                                (temp->Guitars.Year > pFwd->Guitars.Year && Key == 3) ||
                                                (pFwd->Guitars.Wood.Neck[0] > temp->Guitars.Wood.Neck[0] && Key == 4) ||
                                                (pFwd->Guitars.Wood.Deck[0] > temp->Guitars.Wood.Deck[0] &&
                                                 Key == 5)); pBwd = pFwd, pFwd = pFwd->next);
        if (!pBwd) {
            temp->next = sort;
            sort = temp;
            sort->prev = NULL;
...
LIST processingMenu(LIST list) {
    ...
    if (menuItem == 1 || menuItem == 4 || menuItem == 5) {
        printf("Введите по какому %s выводить результаты - ", str);
        tempstr = enterWord();
    }
    if (menuItem > 0 && menuItem < 6)
        New_list = search(list, menuItem, temp, tempstr);
    if (count(New_list))
        messages(5);
    else
        messages(6);
    ...
    for (; list; list = list->next)
        if ((!strcmp(list->Guitars.Name, str) && Key == 1) || (list->Guitars.Strings <= num && Key == 2) ||
            (list->Guitars.Year <= num && Key == 3) || (!strcmp(list->Guitars.Wood.Neck, str) &&
                                                        Key == 4) ||
            (!strcmp(list->Guitars.Wood.Deck, str) && Key == 5)) {
            pFwd = search;
